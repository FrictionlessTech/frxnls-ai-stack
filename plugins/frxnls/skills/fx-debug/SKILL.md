...
<bug_description> <user-provided input> </bug_description>
...
- Other trackers (Linear, Jira, any tracker URL): attempt to fetch via an available
  MCP tool or web access. If the fetch fails — auth, missing tool, non-public page —
...
Then offer next steps via a direct question to the user. Do not assume the user wants action right
...
3. **Rethink the design** (`$fx-brainstorm`) — only when the root cause reveals a
...
|---------|-----------|-----------|
| Hypotheses point to different subsystems | Architecture/design problem, not a localized bug | Present findings, suggest `$fx-brainstorm` |
...
clearly independent subsystems, dispatch read-only sub-agents through Codex subagent delegation in
...
`$fx-brainstorm` and this skill ends.
...
- If the current branch is the default branch, ask (via a direct question to the user) whether to
  ...
  (`codex/fix-<slug>`) and run `git checkout -b <name>`. On any other branch, proceed.
...
6. Self-review the diff before declaring the fix done: read every changed line and check
   ...
   the lightweight `/code-review` — not the full `$rex-code-reviewer` PR-tier flow,
...
Phase 3. Ask via a direct question to the user (don't end the phase without a response); the option
set differs by case:
...
When offering, use a direct question to the user. If the user accepts, run `$fx-compound`
...
- `$fx-learnings-research` — the agent that searches `docs/solutions/` for a prior
...
reference. Manual override: `$fx-debug [issue ref | error | test path]`.
