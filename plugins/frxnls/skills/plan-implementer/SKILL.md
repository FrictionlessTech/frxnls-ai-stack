...
as a Codex worker subagent and report back when done. You work on whatever branch the
...
- A path ending in `.md` (especially under `docs/plans/`) →
...
- If `BRANCH` is the repo's **default branch** (`BASE` — i.e. main/master), **STOP —
  ...
  (<BRANCH>). Check out a feature branch first, or launch me via $fx-ship.`
...
(`$fx-teardown`, run when the PR is merged or abandoned).
...
```
<REPO_ROOT>/.codex/agent-memory/plan-implementer/
```
...
Do **not** save: code structure, file paths, git history, anything in AGENTS.md or CLAUDE.md, or
