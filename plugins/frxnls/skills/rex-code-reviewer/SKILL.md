...
- Do not store facts that can be recovered cheaply from the repo, git
    history, or AGENTS.md/CLAUDE.md files.
...
**Codex model policy.** Let Codex choose each review subagent's model unless the user explicitly requests one. Keep the review lanes and fan-out policy below unchanged.
...
`rolls/partition → 2` makes it 6 passes; security stays cheapest at 1.
...
#### Subagent 1 — Code Simplicity (model selected by Codex session policy)
...
#### Subagent 2 — Security (model selected by Codex session policy)
...
#### Subagent 3 — Documentation (model selected by Codex session policy)

Determine whether documentation should be updated.

Discovery — search repo for: AGENTS.md, CLAUDE.md, README.md, `docs/`, module READMEs, ADRs, runbooks, changelogs, OpenAPI/schema docs, migration/config docs.
...
#### Subagent 4 — Correctness & Runtime Safety (model selected by Codex session policy)
...
#### Subagent 5 — Data Integrity, Contracts & Migrations (model selected by Codex session policy)
...
Only write memory when all of these are true:
 ...
 2. It is not easily derivable from the repo, git history, or AGENTS.md/CLAUDE.md files.
...
Bad candidates:
 ...
 • Anything already documented in AGENTS.md or AGENTS.md/CLAUDE.md files
...
```
<REPO_ROOT>/.codex/agent-memory/rex-code-reviewer/
```
