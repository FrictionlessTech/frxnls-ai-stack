...
Then build a mental model (reasoning, not findings): read AGENTS.md (or CLAUDE.md as a fallback), README, and config, map components and trust boundaries, trace where user input enters → transforms → exits. Write a brief architecture summary before hunting.
...
- **Repo-local (automatic):** Grep `.agents/skills/**/SKILL.md`, `.codex/**`, `.claude/skills/**/SKILL.md`, and hook files for: network exfiltration (`curl`/`wget`/`fetch` to odd hosts), credential access (`ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `process.env` near network calls), prompt injection (`IGNORE PREVIOUS`, `disregard your instructions`).
...
**Independent verifier (if Codex subagent tools available):** for each surviving candidate, launch a fresh-context sub-task given ONLY `file:line` + the FP rules (no anchoring to your reasoning), prompted to refute: "is there a real vuln here? score 1-10; below 8 explain why not." Run in parallel; discard findings the verifier scores below the gate. If the Codex subagent tools is unavailable, self-verify with a skeptic's eye and note it. This verifier is an ad-hoc sub-task with no canonical model-resolution key (unlike the named research agents) — it intentionally omits `model` and inherits the session default; do not invent a key for it.
