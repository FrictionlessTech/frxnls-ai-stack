...
would trip it, and get explicit sign-off via a direct question to the user before spending a
...
`references/lanes.md` for each lane's full brief and hand it to the subagent
...
**Codex dispatch.** Spawn `$fx-lane-researcher` with the built-in `default` role for the seven general lanes and `$fx-risk-researcher` with `default` for risk. Let Codex select models from the current session policy.
...
Spawn the lanes using the role mapping in `../../references/runtime.md`; the specialist skills carry the evidence rules.
