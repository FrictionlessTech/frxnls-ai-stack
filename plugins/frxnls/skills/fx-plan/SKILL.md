...
Ask the user directly in chat; use a structured user-input tool only when one is available in the current mode. One
...
origin doc, or the feature description). Dispatch in parallel through Codex subagent delegation:
...
running it, dispatch a built-in `default` subagent web search with the exact
...
**Handoff.** Present a direct question to the user: "Plan ready at `<absolute path>`. What next?"

- **Ship it** (recommended) — invoke `$fx-ship` with the plan path so it sets
