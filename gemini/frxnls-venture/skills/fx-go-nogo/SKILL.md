...
Run this in the main session — no subagents, no cheaper tier. It is the judgment
...
guessing or inventing the scorecard content.
...
Read `$EXT_HOME/skills/fx-go-nogo/references/scorecard.md` for the rubric. Ten
dimensions, weighted by what `$VENTURE_HOME/constraints.md` says the user
...
Append to `$VENTURE_HOME/_ledger.md` — every venture, whatever the verdict. The
ledger is the compounding asset here: it stops re-litigating dead ideas, and the
accumulated kill reasons sharpen `fx-idea-scout`'s screening over time.

## Step 6 — Handoff

Ask via `ask_user` what happens next. Always ask — never chain automatically.
...
first. There's no point stress-testing the *how* until the *whether* is settled
by real signal.

When the user is ready to build, the venture **leaves this loop**. It stops being
a research folder and becomes a product repo, which is where the delivery
pipeline takes over. That repo doesn't exist yet at decision time, so do not
attempt to start it from here.

Instead, offer to write `$VENTURE_HOME/<slug>/handoff.md` — a portable packet the
user drops into the new repo as `docs/brainstorms/<slug>.md`, where a planning
skill will pick it up as its origin document. It must stand alone without the
research folder:
...
- What the scorecard scored *low* on, carried forward as the known weaknesses
  the planning step should attack first
...
- Reading only `research/_index.md`. The index is a map, not the territory.
