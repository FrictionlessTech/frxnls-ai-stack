---
name: fx-qa-web
description: Systematically QA-test a running web app in a real browser, then fix the bugs found and verify each fix. Use when asked to "qa the site", "test this web app", "find bugs", "test and fix", or "does this work?" for a WEB app. Drives a real browser via the Playwright MCP server. For an iOS app on a simulator, use fx-qa-mobile-ios instead.
allowed-tools:
  - Bash
  - Read
  - Grep
