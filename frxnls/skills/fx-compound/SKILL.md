...
Strip a `mode:headless` token from `$ARGUMENTS` before treating the remainder as a
...
When asking the user anything, use `AskUserQuestion` (load its schema via
`ToolSearch` with `select:AskUserQuestion` first if needed). Fall back to numbered
options in chat only if the tool is unavailable. Never silently skip a question.
...
Also scan the "user's auto-memory" block in your system prompt (Claude Code) for
relevant notes; tag anything you fold in with "(auto memory)".

### Phase 1 — Research (parallel, read-only)

Launch in parallel via the `Agent` tool (skip in Lightweight mode):
...
`AskUserQuestion` before applying. In **Lightweight** mode, emit a one-line tip
...
Report what was written and learned, then ask "What's next?" via `AskUserQuestion`
...
`/frxnls:fx-plan`) · **Open the doc** · **Flag a stale doc to revise**.
