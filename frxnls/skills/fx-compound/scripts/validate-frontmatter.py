#!/usr/bin/env python3
"""Validate fx-compound docs/solutions/ frontmatter for parser-safety issues.
...
Adapted from EveryInc's compound-engineering ce-compound validator.
"""
import os
import re
import sys
...
def main(argv: list[str]) -> int:
    ...
    # would falsely accept `----` or `---extra` as a terminator and let
    # malformed docs slip through to downstream parsers.
    lines = text.split("\n")
    ...
    # fields. Scan line-by-line and only flag top-level mapping entries
    ...
    for lineno, line in enumerate(fm_text.split("\n"), start=2):
        ...
        if not val_stripped:
            ...
            # block (`tags:` followed by `- foo`). Nothing to validate.
            continue
