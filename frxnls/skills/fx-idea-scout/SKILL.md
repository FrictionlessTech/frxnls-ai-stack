...
argument-hint: "[optional: idea, vertical, category, or constraint like '$5k MRR, low maintenance']"
allowed-tools:
  - Bash
  - Read
  - Write
...
this is a first run. Ask once, via `AskUserQuestion`:
...
3. Seed `_ledger.md` from `${CLAUDE_PLUGIN_ROOT}/templates/ledger-entry.md`
...
`${CLAUDE_PLUGIN_ROOT}/templates/constraints.md` via `AskUserQuestion`: MRR
...
Spawn via `Agent(fx-screen-scout)`, resolving the model first:

```bash
SCREEN_MODEL=$("${CLAUDE_PLUGIN_ROOT}/scripts/resolve-model.py" fx-screen-scout --repo-root "$VENTURE_HOME")
```

Defaults to `haiku`. This step is broad, shallow retrieval — it doesn't need a
frontier model, and the whole point of the screen is that it's cheap. If you find
it grading thin evidence as **Observed**, retier it to `sonnet` in
`$VENTURE_HOME/.frxnls/model-tiers.json` rather than editing the plugin.
...
Then use `AskUserQuestion` to let them pick 1–3 to research. Offer "none of
...
`${CLAUDE_PLUGIN_ROOT}/templates/brief.md`. This is the sequential step that everything downstream
depends on — the parallel research lanes are meaningless without a fixed segment
and ACV assumption, so the brief must pin down:
