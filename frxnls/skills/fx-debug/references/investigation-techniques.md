...
### Node.js / TypeScript
- Async stack traces: run with `--async-stack-traces` for full async call chains
...
- Re-render loops: check effect dependency arrays and unstable callback/object props; log render counts
- Stale closures: a handler capturing an old state/prop value — verify with a log inside the handler vs render
- Metro/bundler staleness: clear the cache (`expo start -c`) when a change isn't reflected — a frequent false "it didn't fix it"
- Native vs JS boundary: a value correct in JS but wrong on-device often means a serialization or bridge assumption
...
### Postgres / Supabase
- Query plan: `EXPLAIN (ANALYZE, BUFFERS)` on the suspect query — expected index or a seq scan?
- RLS: a row "missing" for one role but present for another is usually a row-level-security policy, not a data bug
- Migration drift: confirm the live schema matches the migrations (frxnls runs migrations through Drizzle) — a column the code expects but the DB lacks
- Connection pool exhaustion / leaked connections under load
...
|----------|------------------------|---------------------------|
| Python | `breakpoint()` in code, or `python -m pdb script.py` | `python -m pdb -p <pid>` (Python 3.14+ only); on earlier versions, instrument with `rpdb` / `remote-pdb` and connect after it triggers |
...
For test runs, most test runners integrate with the above — e.g., `node --inspect-brk $(which jest)`, `pytest --pdb`. Prefer the runner's integration over trying to attach post-hoc.
...
## Browser and Simulator Debugging

frxnls drives a real browser through the **Playwright MCP server** (the tool `fx-qa-web`
uses) and the iOS Simulator through **serve-sim** (the tool `fx-qa-mobile-ios` uses).
Reproduce UI bugs against the live surface rather than reasoning from the component tree
alone.

**Web (Playwright MCP):**

- Navigate to the affected route (`browser_navigate`), then capture the accessibility
  tree (`browser_snapshot`) — it gives stable element refs to act on.
- Interact (`browser_click`, `browser_type`, `browser_fill_form`) and re-snapshot to see
  the state after each step.
- **Console errors** (`browser_console_messages`): JavaScript errors, failed requests,
  and CORS issues often reveal the root cause before any code tracing.
- **Network** (`browser_network_requests`): a 422 or 500 from the backend narrows the
  investigation immediately — the bug is server-side, not in the UI.
- Save visual evidence with `browser_take_screenshot` when the symptom is visual.

**Port detection:** check project instruction files (`AGENTS.md`, `CLAUDE.md`) for port
references, then `package.json` dev scripts, then `.env`, falling back to `3000` (or the
worktree's allocated port from `fx-expo-worktree-dev` for an Expo web target).

**iOS / Expo (serve-sim):** boot the app with `fx-expo-worktree-dev`, then drive the sim
through serve-sim — read the accessibility tree, tap/gesture/type, and tail device logs.
A crash or red-box stack trace in the device log is usually the fastest path to the
failing frame; correlate it back to the JS source.
...
**Correlation IDs.** Most web frameworks either attach a request ID automatically or accept one via header (`X-Request-ID`, `traceparent`). When the project has one, every log line and every downstream call should carry it. If it's missing or not propagated, that is itself a finding.
...
- Locks and transactions: inspect the lock/transaction tables (`pg_locks`, `information_schema.innodb_trx`) — is the operation waiting on a long-held lock?
...
- **Cache staleness:** correct behavior immediately after a change, wrong behavior after some time, fixed by restart or cache flush; includes HTTP caches, CDN caches, app-level memoization, browser service workers, Metro bundler cache
- **Permissions / auth:** works for one user and not another, works in dev without auth layer but fails in prod with it, works with superuser but not with the actual operating identity; for Postgres, suspect RLS
...
- **Stale build artifacts:** `dist/`, `.next/`, compiled `.pyc`, generated code, Metro/Expo cache, Docker image layers — rebuild from clean and see if it reproduces
